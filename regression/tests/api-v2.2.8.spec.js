...
test.describe('V2.2.8 接口回归', () => {
  ...
  test('超时递交筛选：is_over_tb_time=1 为全量的真子集', async ({ request }) => {
  ...
  test('已知问题跟踪：is_over_tb_time 非法枚举不校验（人工确认表 #2）', async ({ request }) => {
  ...
  test('人员看板：type=pm/dta/tech 正常、非法 type 优雅报错', async ({ request }) => {
  ...
  test('递交常量：WDPAPI 枚举含 API2.3.x', async ({ request }) => {
  ...
  test('需求状态枚举：#6690 存在自动暂停的 pause 需求', async ({ request }) => {
    const j = await (
  ...
  test('模型看板总览：发包/金额汇总自洽', async ({ request }) => {
    const j = await (
  ...
  test('模型看板明细：非法筛选参数不 5xx', async ({ request }) => {
    const cases = [
  ...
  test('已知BUG跟踪：资产明细负数 limit 触发 500（人工确认表 #6）', async ({ request }) => {
