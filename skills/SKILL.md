...
6. **结论分级（三条正交轴，不混用一个符号）**：① **验收结论/需求**：✅满足 / 🟡部分满足 / ❌不满足（写在 §一 各需求节标题，不另列汇总表）；② **缺陷 BUG**：已确认必修，含复现+预期 vs 实际+严重度（写 §二）；③ **风险 Risk**：按影响×可能性定级（写 §三），口径待定/覆盖盲区/健壮性仅用于判断、不单列成表列。关键："没验到"是风险里的覆盖盲区、"待产品确认口径"是风险，都不是缺陷。
