...
5. **截图纪律**：只截三类——关键结果页（每功能 1~2 张）、BUG 现场（必截）、定妆图 `final-{功能名}.jpg`（每功能 1 张，发版文档引用）。落盘 `acceptance/{版本}/`。
   - **视口硬规定**（反例：定妆图曾被裁掉右侧 1/4）：集成浏览器新页签默认 1280×720，51PM 按 1920 宽设计，右侧栏会溢出视口被裁。每开新页签**第一件事** `page.setViewportSize({width:1920,height:1080})`；每次截图前先断言本图的关键元素 `getBoundingClientRect()` 在视口内，再 `page.screenshot()`。小视口还会导致左侧仪表盘菜单不渲染。
