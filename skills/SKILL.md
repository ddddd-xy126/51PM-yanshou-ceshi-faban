...
5. **截图纪律**：只截三类——关键结果页（每功能 1~2 张）、BUG 现场（必截）、定妆图 `final-{功能名}.jpg`（每功能 1 张，发版文档引用）。落盘 `acceptance/{版本}/`。
   - **视口硬规定**（反例：定妆图曾被裁掉右侧 1/4）：集成浏览器新页签默认 1280×720，51PM 按 1920 宽设计，右侧栏会溢出视口被裁。每开新页签**第一件事** `page.setViewportSize({width:1920,height:1080})`；每次截图前先断言本图的关键元素 `getBoundingClientRect()` 在视口内，再 `page.screenshot()`。小视口还会导致左侧仪表盘菜单不渲染。⚠️ setViewportSize 可能静默不生效（V2.2.7 轮：innerWidth 实际仅 1395，截图 1920 画布右/下大片留白）——截图前先断言 `window.innerWidth===1920`，不成立改用 Playwright 临时脚本重截（细则见 release_acceptance.md 截图规范）。
...
| 阶段 4                        | 发版初稿 → 人工定稿；归档前确认最终稿（产物评审，非流程中断） |
