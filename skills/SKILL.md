...
description: "51PM 验收-测试-发版全流程总控 SKILL（Copilot + Playwright）。输入一段开发者本周开发内容 + 版本号，即可驱动完整流水线：老功能 Playwright 回归 → 新功能真实 UI 验收出报告 → 走通路径沉淀为回归用例 → 生成发版内容初稿（写进验收报告，定稿归档由用户自行完成）。触发词：验收、回归、发版、走一遍流程、验收 V2.x.x。"
...
> **① 回归（老功能）→ ② 验收（新功能）→ ③ 沉淀（用例+入口）→ ④ 发版（初稿写进报告）**，
...
graph TD
    ...
    C --> D[阶段2 新功能验收<br>拆清单→直接开跑→真实UI走流程]
    D --> E[阶段3 沉淀<br>走通路径→新 spec · 新入口→entry_map]
    E --> F[阶段4 发版<br>报告内生成初稿→交付终点<br>定稿归档由用户在发版管理自行完成]
```
...
| 其他用例意外红                            | 疑似回归 BUG                                               | 先复跑一次排除偶发（V2.2.6 轮需求页偶发空载先例）；仍红则看现场，作为 🐛 记入报告继续往下跑，**不中断等用户** |

- 回归失败排查顺序：登录态过期（批量跳登录页）→ 8888 转发没起 → 测试库刷新致数据缺失 → 才是真回归 BUG。
- **测试库刷新致数据缺失**：失败报错含「测试数据缺失 / 被清空需先重建」（先例：#6712 项目、测试账号任务、V225copilot 发包被整库刷新清掉）时，**不要重建数据、不要复跑、不要继续耗时间**——立即终止本轮回归，在报告开头标注「回归跳过：测试库刷新致数据缺失」并列出受影响用例，直接进阶段 2。数据重建只在用户明确要求时做。
...
4. **四层覆盖**：每个功能默认过「UI 流程 / 边界 / 接口层 / 数据一致性」，验不了的层在报告标 ⚠️+原因。其中接口层是**硬性要求**：每个功能至少直调 1 次核心接口（页面内 `fetch` 复用会话）验边界参数（非法值、越权 id、空参），并把接口路径+关键参数记进报告与 entry_map 备注（供阶段 3 沉淀接口回归用），不再是“必要时”。
5. **截图纪律**：只截三类——关键结果页（每功能 1~2 张）、BUG 现场（必截）、定妆图 `final-{功能名}.jpg`（每功能 1 张，发版文档引用）。落盘 `acceptance/{版本}/`。
   - **交付截图一律用 headless Playwright 脚本截，禁止用集成浏览器出图**。根因：集成浏览器视口受面板物理尺寸限制，`setViewportSize(1920)` 后 `innerWidth` 瞬时返 1920 但会被 VS Code **异步静默回弹**到 ~1390，截图右 1/4 会留白返工。集成浏览器只做探索交互；验收尾声用 `regression/scripts/headless-login.js`（launchLoggedIn+shot，登录/公告/视口断言已封装）写一个重放脚本把全部交付截图一次截完（只读重放；写操作已完成的场景可接口推回前置状态→截图→重演）。
...
2. **初稿写进验收报告**的「## 发版内容（初稿，待人工定稿）」节——不落独立文件；定妆图直接引用 `acceptance/{版本}/final-*.jpg`，不拷贝。写完再转 html（见阶段 2 第 8 条）。**转完 html 即交付终点**，定稿与归档由用户自行完成。
3. 若用户主动提供定稿与初稿的措辞差异，作为反例回填 release_notes.md 相应章节；若用户修改了报告 md，重转一次 html 保持交付物同步。
...
> 「不问直接干」四条：① 环境固定测试环境；② 验收清单输出后直接开跑；③ 回归/验收发现**测试数据缺失**（如 #6712 需求被清理）直接补建数据重跑；④ 疑似回归 BUG 先复跑排除偶发，确认后记入报告继续跑，不中断。
