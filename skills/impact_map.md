...
| 拆解项目反馈/逃逸Bug（V2.2.2）| `produce_demand`（反馈拆解，反馈模块枚举含逃逸Bug）、`demand`、`project_task`（拆解生成任务） |
| 项目反馈递交流程（V2.2.2）    | `project_publish`（TB申请递交/PM审批递交/QA递交/递交排期/状态追踪全链）、`produce_demand`（反馈提交/拆解/验收） |
| 项目-特殊事项提醒（V2.2.2）   | `project_detail`（项目编辑表单 特殊事项需同步/特殊事项提醒 + 列表提示图标）        |
| 排期模式多选创建（V2.2.2）    | `schedule`（排期表进入排期模式，拖选同行连续单元格创建任务）                        |
| 任务时间筛选扩展（V2.2.2）    | `project_task`（任务列表 任务时间/花费日期 两种时间筛选方式）                       |
| 递交时间轴（V2.2.2）          | `project_publish`（递交列表 递交排期时间轴模块）                                   |
| 递交列表-程序开发筛选（V2.3.1）| `project_publish`（get_list 新增 `chengxu_people` 程序开发人员筛选参数）           |
| 模型外包-外包筛选（V2.3.1）    | `outsource`（get_package_list `is_self_made=0/1` 外包/自制对称筛选）               |
...
项目递交（项目内）· 递交列表→项目跳转 · PM审批递交申请 · QA递交（状态自动判定）· 递交列表超时递交筛选 · TB递交申请 · 申请递交表单（V2.2.9 重构）· 我的递交排期日历（V2.2.9）· 递交排期日历视图/季度（V2.2.9）· 项目反馈递交流程（V2.2.2 源头）· 递交时间轴（V2.2.2）· 递交列表-程序开发筛选（V2.3.1）
...
需求拆分 · 需求任务列表弹窗 · 批量创建任务 · 填写工时 · 任务完工 · 我的任务（日历）· 任务卡备注 · 创建任务快速建组群 · 任务时间筛选扩展（V2.2.2）· 任务列表（V2.2.9 项目/非项目共用容器重构）· 我的项目/非项目视图（V2.2.9 侧栏合并）· 非项目层级重构（V2.3.0）· 我的任务日历右侧栏重构（V2.3.0）
...
排期表搜索项目 · 排期表过滤空白行列 · 查看个人排期 · 排期模式多选创建（V2.2.2）
...
项目测试文档 · 行业接口文档 · 定制接口文档 · 项目概况-预估营收时间（V2.2.9）· 项目-特殊事项提醒（V2.2.2）· 项目概览时间轴-预估营收（V2.3.1）
