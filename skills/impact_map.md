...
项目递交（项目内）· 递交列表→项目跳转 · PM审批递交申请 · QA递交（状态自动判定）· 递交列表超时递交筛选 · TB递交申请

> 改递交状态判定 / 递交筛选 / 递交审批任一处，整簇联动风险高——递交状态枚举、is_over_tb_time、审批流转互相耦合。
...
需求拆分 · 需求任务列表弹窗 · 批量创建任务 · 填写工时 · 任务完工 · 我的任务（日历）· 任务卡备注 · 创建任务快速建组群

> 任务字段/状态/指派人改动波及工时、日历、完工、需求下钻。⚠️离职人员 assigned_to 保留（V2.2.7 修复点）。

### `outsource` + `outsource_feedback` + `supplier_api`（模型外包全链）

申请发包 · 发包审核/立项 · 外包反馈管理 · 反馈验收工作台 · 模型数据看板 · 供应商企业管理 · 供应商端入口

> 管理端建的项目/发包/任务实时同步供应商门户；反馈 quality_status 0/1/2/3 与验收工作台强耦合；自制 vs 供应商分支（isSelfMade）影响反馈 tab 显隐。
...
> 工时数据是三者共同数据源，工时录入口径改动波及统计与导出。

### `project_detail`（项目概况文档位）

项目测试文档 · 行业接口文档 · 定制接口文档

> 三个文档位同区域、互不覆盖是回归重点（历轮易出「链接互相覆盖」类 BUG）；V2.2.9 起移入折叠「项目信息」手风琴面板。
...
3. **跑目标集**：因为目标集小，这几个老功能优先写成/改成**动作型自给自足**用例（自造前置→执行→验证），不依赖遗留数据、不 skip。
4. **兜底冒烟**：跑全部 `api-*.spec.js`（纯接口不开浏览器，秒级），验后端契约防漏判；UI 全量仅在预判维度普遍拿不准或大范围重构时才跑。
