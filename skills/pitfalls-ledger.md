...
| P-15 | AI 功能验收：`ai_skill/execute` 等 AI 接口返 **HTTP 200 + `code:0` 但 `data.status:"llm_error"`**（reply=「模型服务请求过于频繁 429」等），误判为成功；或死等 LLM 成功却因模型配额限流反复 429 耗时 | AI 走异步 agent 任务，后端契约层成功 ≠ 模型服务成功；模型（deepseek-v3）共享配额易 429 | 断言 AI 成功要看 **`data.status`（ready 才算成功、llm_error/429 是失败）**，不能只看 HTTP/`code`；哨兵用例只守接口契约（不 5xx + 返 `data.status`），**别死等 LLM 成功**；真实回填质量在模型配额充足时段人工复验，持续 429 记为环境盲区（非代码缺陷）写风险 | 1 | 2026-08-17 |
